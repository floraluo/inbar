  ...
  <div>
    <div class="page-main">
      <div class="btn-operate-group">
        ...
        <button class="btn btn-primary" @click="openImportBillLayer"><i class="iconfont icon-daoru"></i>导入</button>
      ...
      <div class="goods-search-bar">
        <div class="search-group">
          ...
          <!--<input v-model="searchGoodsNameOnly" class="form-control" type="text" placeholder="请输入商品名称按Enter进行检索" @keyup.enter="searchGoodsByName">-->
          <!--<ul class="search-result">-->
            <!--<li v-for="item in searchGoodsList" :key="item.id">{{item.goodsName}}</li>-->
          <!--</ul>-->
        </div>
      </div>
      <div class="goods-info-bar" v-if="tableUpdate.goodsList.length > 0">
        <strong class="center">共  <span>{{tableUpdate.goodsList.length}}</span>  条数据； <span>{{updateGoods.amount}}</span>  件商品；补货总金额  <span>{{updateGoods.total}}</span>  元</strong>
        <div class="bottom-btn-group center">
          <button class="btn btn-primary" @click="submitUpdateGoods">确定</button>
    ...
    <!--添加补货商品-->
    <div class="layer-open layer-add-update-goods" id="layerAddUpdateGoods">
      ...
      return {
        ...
        tableUpdate: {
          ...
          goodsColumns: [
            ...
            {field: 'goodsCostprice', title: '进价/元', width: 100, titleAlign: 'center', columnAlign: 'center', isResize: true, isEdit: true,
            ...
            {field: 'updateAmount', title: '补货数量', width: 100, titleAlign: 'center', columnAlign: 'center', isResize: true, isEdit: true,
              formatter: (rowData, rowIndex, pagingIndex, field) => {
                return `<span style="display: none">${rowData[field]}</span><input class="cell-edit-input" type="text" style="text-align: center; width: 100%; height:80%" value="${rowData[field]}"/>`
              }
            },
      ...
      submitUpdateGoods() {
        ...
        if (this.tableUpdate.goodsList.length > 1) {
          query = this.tableUpdate.goodsList.reduce((result, item, currentIndex) => {
            if (currentIndex === 1) {
              return `barcode=${result.goodsBarcode}&nums=${result.updateAmount}&price=${result.goodsCostprice}&barcode=${item.goodsBarcode}&nums=${item.updateAmount}&price=${item.goodsCostprice}`;
            } else {
              return `${result}&barcode=${item.goodsBarcode}&nums=${item.updateAmount}&price=${item.goodsCostprice}`;
            }
          })
        } else {
        ...
        if (this.tableUpdate.goodsList.length > 1) {
          query = this.tableUpdate.goodsList.reduce((result, item, currentIndex) => {
            if (currentIndex === 1) {
              ...
              &price=${result.goodsCostprice}
              &unit=${result.unit}
              &classify=${item.goodsBarcode}
              &name=${item.goodsName}
              &nums=${item.updateAmount}
              &price=${item.goodsCostprice}
              &unit=${item.unit}`;
            } else {
              return `${result}&classify=${item.goodsBarcode}&name=${item.goodsName}&nums=${item.updateAmount}&price=${item.goodsCostprice}&unit=${item.unit}`;
            }
          })
        } else {
          const goods = this.tableUpdate.goodsList[0];
          query = `classify=${goods.goodsBarcode}&name=${goods.goodsName}&nums=${goods.updateAmount}&price=${goods.goodsCostprice}&unit=${goods.unit}`
        }
    ...
    return GET('/api/stock/goodsStock')
      .then(data => {
        ...
        if (vm.annunciator.barEnabled) {
          vm.tableUpdate.goodsColumns.splice(4, 0, {field: 'goodsNumberB', title: '吧台库存', width: 70, titleAlign: 'center', columnAlign: 'center', isResize: true,
            formatter: (rowData) => {
