        ...
        <div class="row">
          <div class="col-xs-12 col-md-4">
            <div class="panel figure-panel"><strong class="figure">￥{{fund.originalAmount}}</strong>
              ...
              <div class="bar red">
                <i v-if="fund.originalAmount >= fund.actualAmount" style="width: 100%"></i>
              </div>
            </div>
          </div>
          <div class="col-xs-12 col-md-4">
            <div class="panel figure-panel">
              <div class="flex-box">
                <strong class="figure">￥{{fund.actualAmount}}</strong>
          ...
          <div class="col-xs-12 col-md-4">
            <div class="panel figure-panel"><strong class="figure">￥{{fund.differenceAmount}}</strong>
              ...
              <div class="bar orange">
                <i v-if="fund.originalAmount !== +fund.actualAmount" :style="`width: ${diffWidth}`"></i>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      diffWidth() {
        if (this.fund.originalAmount - this.fund.actualAmount > 0) {
          if (this.fund.actualAmount === 0) return '100%'
          return (Math.abs(this.fund.actualAmount / this.fund.originalAmount) * 100) + '%'
        } else {
          if (this.fund.originalAmount === 0) return '100%'
          return (Math.abs(this.fund.originalAmount / this.fund.actualAmount) * 100) + '%'
        }
      }
    },
    methods: {
    ...
    created () {
      vm = this;
      GET(`/api/to-boss/cash/${this.$route.query.id}`)
        .then(data => {
          vm.fund = data;
