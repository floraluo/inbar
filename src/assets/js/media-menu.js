...
$.site.menubar = {
  ...
  init: function () {
    ...
    this.tabId = $('#admui-navbar').find('li.highlight > a').attr('data-href');

    if (this.tabId === '#') {
      this.tabId = $('.nav-tabs li.active').find('ul>li.active>a').attr('data-href');
    }
    ...
    $('.nav-tabs li:not(.no-menu)').on('shown.bs.tab', function (event) {
      var tabId = self.tabId = $(event.target).attr('data-href');
